          ...
          <div class="col-12">
            <div class="card">
                <div class="card-header" style="background-color: #007BFF">
                    <h3 class="card-title" style="color:white">Available Fund Requests</h3>

                <div class="card-tools">
                  <div class="input-group input-group-sm" style="width: 150px;">
                    <input type="text" name="table_search" id="search_string" class="form-control" placeholder="Search">
              ...
              <div class="card-body table-responsive p-0" style="height: 390px;">
                <table class="table table-head-fixed table-hover text-nowrap">
                  ...
                  <tbody>
                    <?php
                        $query = "SELECT fund.request_id,
                            user.user_id,user.email,
                            user.user_name,
                            user.designation_name,user.phone,
                            fund.request_date,
                            fund.amount,
                            fund.reason,fund.status,
                            fund.request_handler 
                            FROM fund,user
                            WHERE
                            fund.employee_id = user.user_id
                            ORDER BY fund.request_date DESC, fund.request_id DESC";
                        $result = mysqli_query($con,$query);
                        while($row=mysqli_fetch_array($result))
                          ...
                          echo "<tr>
                            ...
                            if($row['status']==0){
                                if($row['email']!=$_SESSION['email']){
                                    echo "<td> 
                                    <a data-toggle='dropdown' href='#'><i style='color:green' title='Approve Request' class='far fa-check-square approve-request'></i></a><input hidden type='text' value='".$row['request_id']."'>
                                    <a data-toggle='dropdown' href='#' style='margin-left:20px'><i style='color:red' title='Reject Request' class='far fa-times-circle reject-request'></i></a>
                                   </td>";
                                }
                                else{
                                    echo "<td></td>";
                            ...
                            /*echo "
                            <td><a data-toggle='dropdown' href='#'><i style='color:green'  class='far fa-check-square approve-request'></i></a><input hidden type='text' value='".$row['request_id']."'></td>

                            <td><a data-toggle='dropdown' href='#'><i style='color:darkred'  class='far fa-times-circle reject-request'></i></a><input hidden type='text' value='".$row['request_id']."'></td>

                            
                            <td><a data-toggle='dropdown' href='#'><i style='color:red'  class='far fa-trash-alt delete-request'></i></a><input hidden type='text' value='".$row['request_id']."'></td>
