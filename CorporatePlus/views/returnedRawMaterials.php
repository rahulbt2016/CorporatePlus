<?php
    ...
    if(!$_SESSION['email']){
        header("Location: http://localhost/CorporatePlus/views/login.php");
    }
    ...
    if($flag==0){
        header("Location: http://localhost/CorporatePlus/views/dashboard.php");
    }
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Returned Raw Materials</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
              <li class="breadcrumb-item active">Returned Raw Materials</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header" style="background-color: #007BFF">
                <h3 class="card-title" style="color:white">List of Returned Raw Materials</h3>

                <div class="card-tools">
                  <div class="input-group input-group-sm" style="width: 250px;">
                    <input type="text" id="returned-raw-mat-search-string" name="table_search" style="margin-left:20px" class="form-control" placeholder="Search">

                    <div class="input-group-append">
                      <button type="submit" class="btn btn-default" id="returned-raw-mat-search-button"><i class="fas fa-search"></i></button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="card-body table-responsive p-0" style="height: 390px;">
                <table class="table table-head-fixed table-hover text-nowrap">
                  <thead>
                      <tr>
                      <th>Raw Material</th>
                      <th>Quantity<br/>Returned</th>
                      <th>Vendor<br/>Reg. Number</th>
                      <th>Unit<br/>Cost</th>
                      <th>Batch Number</th>
                      <th>Return Date</th>
                      <th>Returned By</th>
                      <th>Reason to Return</th>
                      <th>Capital<br/>Reclaimed</th>
                      
                      
                    </tr>
                  </thead>
                  <tbody id="returned-raw-mat-table">
                      <?php
                        $query = "SELECT raw_materials.name as raw_mat_name,returned_raw_materials.returned_quantity,vendor.vendor_reg_num,raw_material_orders.unit_cost,raw_material_orders.batch_number, DATE_FORMAT(returned_raw_materials.return_date,'%e %b, %Y') as return_date, user.user_reg_num,returned_raw_materials.reason FROM returned_raw_materials,raw_materials,vendor,raw_material_orders,user WHERE returned_raw_materials.raw_mat_order_id=raw_material_orders.raw_mat_order_id AND returned_raw_materials.returned_by=user.user_id AND raw_materials.raw_mat_id=raw_material_orders.raw_mat_id AND vendor.vendor_id=raw_material_orders.vendor_id ORDER BY returned_raw_materials.return_date DESC,returned_raw_materials.returned_raw_mat_id DESC";
                        $result = mysqli_query($con,$query);
                        while($row=mysqli_fetch_array($result)){
                            echo "<tr>";
                        ...
                        /*
                        $query = "SELECT * FROM returned_raw_materials ORDER BY return_date DESC";
                        $result = mysqli_query($con,$query);
                        while($row=mysqli_fetch_array($result)){
                            
                            $query1="SELECT * FROM raw_material_orders where raw_mat_order_id=".(int)$row['raw_mat_order_id'];
                            $result1 = mysqli_query($con,$query1);
                            while($row1=mysqli_fetch_array($result1)){
                                
                                $query3 = "SELECT * from raw_materials WHERE raw_mat_id=".$row1['raw_mat_id'];
                                ...
                                $query4 = "SELECT * from vendor WHERE vendor_id=".$row1['vendor_id'];
                                $result4 = mysqli_query($con,$query4);
                                while($row4=mysqli_fetch_array($result4)){
                                    $vendorRegNum = $row4['vendor_reg_num'];
                                    break;
                                }
                                
                                $unitCost = $row1['unit_cost'];
                                $batchNum = $row1['batch_number'];
                                
                                break;
                            }
                            
                            $query2 = "SELECT user_reg_num from user WHERE user_id=".$row['returned_by'];
                            $result2 = mysqli_query($con,$query2);
                            while($row2=mysqli_fetch_array($result2)){
                                $returnedBy = $row2['user_reg_num'];
                                break;
                            }
                            
                            
                            echo "<tr>";
                                echo"<td>".$rawMatName."</td>";
                                echo"<td>".$row['returned_quantity']."</td>";
                                echo"<td>".$vendorRegNum."</td>";
                                echo"<td>₹ ".$unitCost."</td>";
                                echo"<td>".$batchNum."</td>";
                                echo"<td>".date("d M, Y",strtotime($row['return_date']))."</td>";
                                echo"<td>".$returnedBy."</td>";
                                echo"<td>".$row['reason']."</td>";
                                echo"<td>₹ ".$unitCost*$row['returned_quantity']."</td>";
                      ...
                      /*$query="SELECT * FROM raw_material_orders ORDER BY delivery_date DESC";
                      
                      $result = mysqli_query($con,$query);
                      while($row=mysqli_fetch_array($result)){
                        
                        $query2 = "SELECT user_reg_num from user WHERE user_id=".$row['ordered_by'];
                        $result2 = mysqli_query($con,$query2);
                        while($row2=mysqli_fetch_array($result2)){
                        ...
                        $query3 = "SELECT * from raw_materials WHERE raw_mat_id=".$row['raw_mat_id'];
                        $result3 = mysqli_query($con,$query3);
                        while($row3=mysqli_fetch_array($result3)){
                            $rawMatName = $row3['name'];
                            break;
                        ...
                        $query4 = "SELECT * from vendor WHERE vendor_id=".$row['vendor_id'];
                        $result4 = mysqli_query($con,$query4);
                        while($row4=mysqli_fetch_array($result4)){
                            $vendorRegNum = $row4['vendor_reg_num'];
                            break;
                        ...
                        echo "<tr>";
                            ...
                            if($row['quantity_in_stock']>0){
                                ...
                                echo "<td style='color:red'>NIL</td>";
                            }
                            echo"<td>".$vendorRegNum."</td>";
                            echo"<td>₹ ".$row['unit_cost']."</td>";
                            echo"<td>".$row['batch_number']."</td>";
