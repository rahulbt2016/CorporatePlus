<?php
   ...
   if(!$_SESSION['email']){
    ...
    $query="SELECT 
            ...
            WHERE
            products.product_id=".(int)$_SESSION['editProductId']." AND
            product_category.product_category_id=products.product_category_id AND
            user.user_id=products.added_by
...
<!--              <div class="card-header" style="background-color: #007BFF">
                <h3 class="card-title" style="color:white">New Product</h3>
              ...
              <div class="card-body" style="height: auto;">
                ...
                <div>
                    <center>
                        <span>
                        <img id='product-pic' style='height:250px; width:300px' src='../dashboard_assets/dist/product_pic/<?php echo $productImgName ?>' class='elevation-2'>
                        </span>
                        <a  data-toggle='dropdown' href='#'>
                            <i id='edit-product-pic' class='far fa-edit'></i>
                        </a>
                    </center> 
                </div>
                <input type='file' hidden id='new-pro-pic'>  
                  
                <div class="form-group">
                    <label for="exampleInputName1">Product Code</label>   
                    <input type='text' class='form-control' readonly name='product-code' id='product-code' value='<?php echo $productCode ?>'>
                </div>  
                  
                <div class="form-group">
                    <label for="exampleInputName1">Product Name</label>
                    <input type="text" readonly class="form-control validate-input1" value="<?php echo $productName ?>"  id="product-name" placeholder="Enter Product Name">
                    <span  class="invalid-feedback">This field can't be empty</span>
                </div>  
                  
                <div class="form-group">
                  <label>Select Product Category</label>
                  <select id="product-category" class="form-control select2" style="width: 100%;">
                    <?php
                        $result = mysqli_query($con, "select * from product_category");
                        while($row=mysqli_fetch_array($result)){
                            if($productCategoryName == $row['category_name'])
                ...
                <div class="form-group">
                    <label for="exampleInputReason1">Product Description</label>
                    <textarea name='product-description' class='form-control validate-input1' rows='4' id='product-description' placeholder='Enter description of the product'><?php echo $productDescription ?></textarea>
                    <span  class="invalid-feedback">Please enter the product description</span>
                </div>
                
                
                <div class="form-group">
                    <label for="exampleInputFile">Product Documentation (.pdf)</label>
                    <div class="input-group">
                      <div class="custom-file">
                        <input type="file" class="custom-file-input" id="product-documentation">
                        <label class="custom-file-label" for="exampleInputFile">Choose File to Update</label>
                      </div>
                         ...
                         <label for="exampleInputRawMaterials1">Raw Materials</label>
                            ...
                            <div class="card-body">
                              <div class="row">
                                <div class="col-12">
                                  <div class="form-group">
                                    <label style="font-weight: normal;">Select Raw Materials that are needed for product manufacturing:</label>
                                    <select class="duallistbox" multiple="multiple" id="pro-raw-mat">
                                      ...
                                      <?php
                                        ...
                                        while($row=mysqli_fetch_array($result)){
                                            ...
                                            $flag = 0;
                                            $result2 = mysqli_query($con, "select * from product_raw_mat where product_id=".(int)$productId);
                                            while($row2 = mysqli_fetch_array($result2)){
                                                
                                                if($row['name'] == $row2['raw_mat']){
                                                    $flag = 1;
                                                }
...
<!--                                    <span  class="invalid-feedback">Please Select atleast 1 raw material</span>-->
                ...
                <div class="form-group">
                    ...
                    <?php echo "<input type='number' value='".$unitCost."' class='form-control validate-input1' value=''  name='cost' min='1' id='unit-cost' placeholder='Enter Unit Cost'>"  ?>
                    <span  class="invalid-feedback">Please enter valid cost</span>
                </div>   
                <br>
                <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                  ...
                  <label class="custom-control-label" for="customSwitch3">Status : Enable?</label>
                </div>  
              ...
              <div class="card-footer">
                  <button  class="btn btn-primary" id="save-edited-product">Save</button>
              </div>
            </div>
            <!-- /.card -->
          </div> 
...
$(document).ready(function () {
 ...
 //Bootstrap Duallistbox
    ...
    $('#edit-product-pic').click(function(){
        $('#new-pro-pic').click();
    });
    
    $('#new-pro-pic').change(function(){
