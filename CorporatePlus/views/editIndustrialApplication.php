...
//    unset($_SESSION['inquiryPageFlag']);
   ...
   if(!$_SESSION['email']){
    ...
    if(!isset($_SESSION['industrialApplicationId']))
        header("Location: dashboard.php");
    
    
    $query="SELECT * FROM industrial_applications WHERE industrial_application_id=".(int)$_SESSION['industrialApplicationId'];
    ...
    while($row=mysqli_fetch_array($result)){
        ...
        $industryDescription = $row['industry_description'];
        $industryImage = $row['industry_image'];
        $active = $row['active'];
           ...
           <div class="col-12">
            <div class="card">
             <div class="card-header" style="background-color: #007BFF">
                <h3 class="card-title" style="color:white">Edit Industrial Application</h3>
              ...
              <div class="card-body" style="height: auto;">
                ...
                <div>
                    <center>
                        <span>
                        <img id='industry-application-pic' style='height:300px; width:350px' src='../dashboard_assets/dist/product_pic/<?php echo $industryImage ?>' class='elevation-2'>
                        </span>
                        <a  data-toggle='dropdown' href='#'>
                            <i id='edit-industry-application-pic' class='far fa-edit'></i>
                        </a>
                    </center> 
                </div>
                <input type='file' hidden id='new-industry-application-pic'>  
                  
                <input type='text' hidden id='industry-application-id' value="<?php echo $_SESSION['industrialApplicationId'] ?>"> 
                
                <div class="form-group">
                    <label for="exampleInputName1">Industry Name</label>
                    <input type="text" readonly class="form-control validate-input1" value="<?php echo $industryName ?>"  id="industry-application-name" placeholder="Enter Industry Name">
                    <span  class="invalid-feedback">This field can't be empty</span>
                </div>  
                
                <div class="form-group">
                    <label for="exampleInputReason1">Description</label>
                    <textarea name='industry-application-description' class='form-control validate-input1' rows='4' id='industry-application-description' placeholder='Enter description of the industry application'><?php echo $industryDescription ?></textarea>
                                    ...
                                    <select class="duallistbox" multiple="multiple" id="industrial-application-products">
                                      ...
                                      <?php
                                            ...
                                            while($row=mysqli_fetch_array($result)){
                                                ...
                                                $result2 = mysqli_query($con,"SELECT product_industrial_application.industrial_application_id,
                                                                            products.product_name 
                                                                            FROM 
                                                                            product_industrial_application,products 
                                                                            WHERE 
                                                                            product_industrial_application.product_id=products.product_id AND 
                                                                            industrial_application_id=".(int)$_SESSION['industrialApplicationId']);
                                                while($row2 = mysqli_fetch_array($result2)){
                                                    if($row2['product_name'] == $row['product_name']){
                                                        $productFlag = 1;
                                                        break;
                                                    }
                                                }
...
<!--                                    <span  class="invalid-feedback">Please Select atleast 1 raw material</span>-->
              ...
              <div class="card-footer">
                  <button  class="btn btn-primary" id="save-edited-industry-application">Save</button>
              </div>
            </div>
            <!-- /.card -->
          </div> 
...
$(document).ready(function () {
  bsCustomFileInput.init();
    ...
    $('#edit-industry-application-pic').click(function(){
        $('#new-industry-application-pic').click();
    });
    
    $('#new-industry-application-pic').change(function(){
    ...
    function readURL(input) {
      if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
          $('#industry-application-pic').attr('src', e.target.result);
        }
